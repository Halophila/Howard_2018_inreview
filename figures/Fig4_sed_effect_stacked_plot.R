...
densityplot <-  sedsummary %>% 
  ggplot(aes(x  = as.factor(sedimentscore_name), y = density_mean)) +
  geom_bar(position = position_dodge(), stat = "identity", colour = 'black') +
...
contentplot <- sedsummary %>% 
  ggplot(aes(x = as.factor(sedimentscore_name), y = trad_LOI_mean))+
  geom_bar(position = position_dodge(), stat = "identity", colour = 'black') +
